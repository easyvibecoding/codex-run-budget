...
予算が有効でない場合も、自動使用量カードは既定で有効です。[ターンごとの記録](docs/AUTO_REPORTS.md)。
...
| ターンごとの自動記録 | 終了前に任意のインラインカードを表示し、ローカルに Stop 記録を保存します。回数を限定した完了確認により、後から別の改訂版を保存できます。 | インラインカードはその時点のスナップショットです。証拠が不足する場合は部分的な状態のままです。 |
...
明示的な無効設定はアップグレード後も残ります。300 秒を超えるターンだけを含めるには `auto-report enable --threshold-seconds 300` を使います。既定のしきい値はゼロです。Stop が最終使用量レコードの保存より先に発生する場合があります。レポート専用 worker はそのターンの完了を確認し、別の改訂版を保存できます。元の Stop ファイルと終了前カードは変更しません。[記録のライフサイクル](docs/AUTO_REPORTS.md)。
