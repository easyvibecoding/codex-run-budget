...
from codex_run_budget.child_usage import (  # noqa: E402
    DB_NAME,
    HEADER_BYTES,
    TAIL_BYTES,
    _load_cache,
...
class ChildUsageTest(unittest.TestCase):
    ...
    def test_collect_rechecks_revocation_after_cache_load_and_fresh_scan(self) -> None:
        for phase in ("after-cache", "after-scan"):
            with self.subTest(phase=phase):
                ...
                with mock.patch(f"codex_run_budget.child_usage.{target}", side_effect=replacement):
                    result = collect(
    ...
    def test_revocation_capacity_is_bounded_without_forgetting_rejection(self) -> None:
        ...
        with mock.patch("codex_run_budget.child_usage.MAX_DB_AGENTS", 1):
