"""Missing prompt-hook recovery through the same Governor interface as Codex."""
...
from codex_run_budget.auto_report import configure, recent
...
class ReportRecoveryTest(unittest.TestCase):
    def setUp(self):
        ...
        for target, value in (("time.time", self.now + 10), ("time.monotonic", 5000),
            ...
            mock = patch("codex_run_budget.auto_report." + target, return_value=value)
            mock.start()
    ...
    def event(self, event="PreToolUse", **extra):
        return Governor.dispatch({**self.payload, "hook_event_name": event, **extra}, self.data)
    ...
    def test_existing_start_or_terminal_never_rereads_or_reinjects(self):
        self.event()
        with patch("codex_run_budget.auto_report.snapshot", side_effect=AssertionError):
    ...
    def test_budget_decisions_and_tool_context_are_preserved(self):
        decisions = [{"decision": "block"}, {"continue": False}, {"hookSpecificOutput": {
            "hookEventName": "PreToolUse", "permissionDecision": "deny"}}]
        for decision in decisions:
            with patch.object(Governor, "_handle_budget", return_value=decision):
                self.assertEqual(self.event(), decision)
                self.assertEqual(recent(self.data), [])
        ...
        self.assertIn("If eligible, before final run this usage-card preview once:", context)
