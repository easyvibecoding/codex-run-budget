...
from codex_run_budget.auto_report import configure, handle, recent
from codex_run_budget.hook_adapter import main as hook_main
from codex_run_budget.reconcile import _DeadlineExpired, run, schedule
from codex_run_budget.util import stable_hash

ROOT = Path(__file__).resolve().parents[1]
HOOK = ROOT / "plugins/codex-run-budget/scripts/hook.py"
...
class ReconcileTest(unittest.TestCase):
    def setUp(self):
        ...
        locale = patch.dict(os.environ, {"CODEX_HOME": str(self.home),
    ...
    def queued(self):
        process = Mock()
        with patch("codex_run_budget.reconcile.subprocess.Popen", return_value=process) as spawn:
    ...
    def test_late_final_counter_waits_for_completion_then_preserves_stop_artifacts(self):
        ...
        with patch("codex_run_budget.reconcile.time.sleep", side_effect=append_late):
    ...
    def test_later_turn_large_counter_is_excluded_from_revision_two(self):
        self.stopped()
        self.queued()
        self.complete()
        ...
        turn = json.loads(self.artifact("json", revised=True).read_text())
        ...
        self.assertEqual(turn["stop_contexts"][0]["model"], "example-model")
        self.assertStopUnchanged()
    ...
    def test_excess_delays_never_expand_the_eight_snapshot_attempt_limit(self):
        self.stopped()
        self.queued()
        from codex_run_budget.auto_report import snapshot

        with patch("codex_run_budget.reconcile.snapshot", wraps=snapshot) as observe:
    ...
    def test_deadline_bypasses_collectors_ordinary_exception_handler(self):
        ...
        with patch("codex_run_budget.reconcile.collect", ordinary_error_collector):
    ...
    def test_real_worker_alarm_interrupts_slow_collector_and_restores_handler(self):
        ...
        script.write_text(textwrap.dedent("""\
            ...
            from codex_run_budget import reconcile
        ...
        child = subprocess.run(
            [sys.executable, "-I", str(script),
             str(ROOT / "plugins/codex-run-budget/lib"), str(self.data), str(self.home)],
    ...
    def test_disabled_reporting_never_spawns_and_queued_worker_exits(self):
        ...
        with patch("codex_run_budget.reconcile.subprocess.Popen") as spawn:
    ...
    def test_disable_while_worker_waits_prevents_publication(self):
        ...
        with patch("codex_run_budget.reconcile.time.sleep", side_effect=disable):
            result = run(self.payload, self.data, home=self.home, delays=(0,))
    ...
    def test_user_edit_during_publication_is_preserved(self):
        ...
        with patch("codex_run_budget.reconcile.os.open", side_effect=write_user_edit):
            result = run(self.payload, self.data, home=self.home, delays=(0,))
    ...
    def test_spawn_error_keeps_stop_hook_successful_and_report_available(self):
        ...
        with patch.object(sys, "argv", [str(HOOK), "--event", "Stop", "--data-dir", str(self.data),
            ...
            sys, "stdin", source
        ), patch("codex_run_budget.reconcile.subprocess.Popen",
