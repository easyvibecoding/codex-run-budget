...
class PublisherUpdatesTest(unittest.TestCase):
    ...
    def test_packaged_signed_runtime_runs_actual_hook_and_cli(self):
        ...
        sessions = self.root / "synthetic-sessions"
        sessions.mkdir()
        report = subprocess.run([sys.executable, str(PLUGIN / "scripts" / script),
                                 "--data-dir", str(self.root / "data"), "report", "window",
                                 "--all-tasks", "--windows", "24h", "--directory", str(sessions),
                                 "--format", "html", "--full"], capture_output=True, text=True,
                                timeout=5)
        self.assertEqual(report.returncode, 0, report.stdout + report.stderr)
        self.assertIn("<!doctype html", report.stdout.lower())
