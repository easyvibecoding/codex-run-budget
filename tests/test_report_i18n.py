...
from codex_run_budget.auto_preview import render_card  # noqa: E402
from codex_run_budget.auto_report import _documents, _pending, handle, recent  # noqa: E402
from codex_run_budget.report_i18n import (  # noqa: E402
    CONFIG_BYTES,
    LOCALES,
    ReportText,
    _catalog,
...
class ReportI18nTest(unittest.TestCase):
    ...
    def test_desktop_override_wins_and_is_reread_without_settings_mutation(self):
        with tempfile.TemporaryDirectory() as temporary, patch(
            "codex_run_budget.report_i18n._system_locale", side_effect=AssertionError("no read")
        ):
            home = Path(temporary)
    ...
    def test_auto_missing_invalid_and_unsafe_config_use_bounded_fallback(self):
        with tempfile.TemporaryDirectory() as temporary, patch(
            "codex_run_budget.report_i18n._system_locale",
    ...
    def test_python310_parser_rejects_fake_sections_and_reads_native_scalar(self):
        ...
        with tempfile.TemporaryDirectory() as temporary, patch(
            "codex_run_budget.report_i18n._tomllib", None
        ):
            home = Path(temporary)
    ...
    def test_macos_ignores_terminal_language_and_read_failure_is_safe(self):
        with tempfile.TemporaryDirectory() as temporary, patch(
            "codex_run_budget.report_i18n.sys.platform", "darwin"
        ), patch.dict("os.environ", {"LC_ALL": "en_US.UTF-8"}), patch(
            "codex_run_budget.report_i18n._mac_language", side_effect=[None, "zh-Hant-TW"]
        ) as reader:
            ...
            self.assertEqual([call.args[0] for call in reader.call_args_list],
                             ["com.openai.codex", "-g"])
        with tempfile.TemporaryDirectory() as temporary, patch(
            "codex_run_budget.report_i18n._system_locale", side_effect=RuntimeError("secret")
        ):
            self.assertEqual(resolve_locale(home=temporary), {
    ...
    def test_locale_switch_between_start_and_stop_preserves_pending_identity(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            data = root / "data"
            with patch("codex_run_budget.auto_report.resolve_locale", return_value={
    ...
    def test_locale_failure_cannot_escape_report_error_handler(self):
        with tempfile.TemporaryDirectory() as temporary, patch(
            "codex_run_budget.auto_report.resolve_locale", side_effect=RuntimeError("fail")
        ):
            ...
            self.assertIn("unchanged", response["systemMessage"])
