...
def build_parser() -> argparse.ArgumentParser:
    ...
    report.add_argument(
        "--all-tasks", action="store_true", help="explicit cross-Task window opt-in"
    )
    report.add_argument(
        "--full", action="store_true", help="print full artifact to stdout instead of saving"
    )
    report.add_argument("--codex-home", type=Path, help="local native Task metadata directory")
    ...
    report.add_argument("--limit", type=int, help="maximum pages/agents (20), or list rows (10)")
    report.add_argument("--detail-limit", type=int, default=20, help="turn/context rows per window")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "report":
        ...
        try:
            ...
            print(
                "run-budget: cannot create report (invalid range, source or output; "
