...
class Ledger:
    def __init__(self, path: Path):
        ...
        self.conn = sqlite3.connect(str(path), timeout=3.0, isolation_level=None)
    ...
    def sync_usage(self, run_id: str, source_id: str | None, usage: Usage) -> dict[str, Any] | None:
        if not source_id:
    ...
    def admit_tool(
        ...
        with self.transaction():
            ...
            if tool_name == "Agent" and int(row["active_agents"]) >= int(row["max_agents"]):
                reason = f"active subagent ceiling reached ({row['max_agents']})"
                return self._block_tool(row, source_id, tool_use_id, tool_name, input_hash, reason)
            if tool_name == "Agent" and ratio >= float(row["block_agents_ratio"]):
                reason = (
            ...
            if repeat_count >= int(row["repeat_steer"]):
                contexts.append(
                    ...
                    "Change strategy and do not retry unchanged."
                )
            return Decision(
    ...
    def complete_tool(self, run_id: str, tool_use_id: str, output_chars: int) -> Decision:
        with self.transaction():
