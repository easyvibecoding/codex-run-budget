...
即使没有启动预算，自动用量卡默认仍会显示。符合条件的子代理可在自己的页面显示用量卡；父 Task 的卡片则单列已观测的后代用量小计。两处以相同的哈希 `@selector` 对应这个子代理。子代理卡片会标出直接亲代，分别标示自身与后代用量；亲代关系冲突时保留未确认状态。[回合记录](docs/AUTO_REPORTS.md)。
