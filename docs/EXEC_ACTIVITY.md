...
python3 plugins/codex-run-budget/scripts/run_budget.py exec-activity watch --project "$PWD" --duration 60
python3 plugins/codex-run-budget/scripts/run_budget.py exec-activity run --project "$PWD" -- --ephemeral "Summarize the repository"
```
...
worktrees are separate scopes. `--codex-home` after the activity action selects a
...
Set `CODEX_RUN_BUDGET_EXEC_ACTIVITY_NOTICES=0` in the Codex host environment to
...
Exec activity is observational. Launcher receipts do not register a native child,
charge the shared budget ledger, or provide enforcement over independent exec
processes. Existing Governor decisions and fail-closed behavior are preserved.
