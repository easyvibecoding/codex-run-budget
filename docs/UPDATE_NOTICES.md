...
From 0.17.0, routine program updates use [signed runtime updates](SIGNED_UPDATES.md).
...
```sh
python3 scripts/run_budget.py updates check --refresh --cwd "$PROJECT_DIR"
python3 scripts/run_budget.py updates check --offline --cwd "$PROJECT_DIR"
```
...
`CODEX_RUN_BUDGET_UPDATE_NOTICES=0` for this plugin alone.
